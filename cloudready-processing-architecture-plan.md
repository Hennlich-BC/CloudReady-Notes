...
├── .env.example
│
├── app/                                 # bind-mount → /app/* внутри контейнеров
│   ├── data/
...
services:
  ...
  worker:
    ...
    environment:
      ...
      NST_CALLBACK_SECRET: ${NST_CALLBACK_SECRET}
      NST_DOCUMENT_TOKEN_TTL_SECONDS: ${NST_DOCUMENT_TOKEN_TTL_SECONDS}
      LOCK_TTL_SECONDS: ${LOCK_TTL_SECONDS}
      RECONCILER_STALE_THRESHOLD_MINUTES: ${RECONCILER_STALE_THRESHOLD_MINUTES}
    volumes:
      - ./app/data/documents:/app/data/documents
      - ./app/templates:/app/templates:ro
      - ./app/config:/app/config:ro
...
Все переменные, на которые ссылается `docker-compose.yml` (раздел 3.1) и которые упомянуты по тексту плана (NST-интеграция — раздел 5.2, безопасность — раздел 8, эксплуатация — раздел 9), сведены в один файл. `.env` — реальные значения, не в git (`.gitignore`), права `600` (раздел 8). `.env.example` — этот же список с плейсхолдерами, коммитится в репозиторий как документация обязательных переменных.
...
# ── RabbitMQ ────────────────────────────────────────────────
RABBITMQ_USER=cloudready
RABBITMQ_PASSWORD=changeme-strong-password

# ── Redis ───────────────────────────────────────────────────
REDIS_PASSWORD=changeme-strong-password

# ── NST (Navision Service Tier) — раздел 5.2 ───────────────
NST_BASE_URL=https://nst.hennlich.local
NST_CALLBACK_SECRET=changeme-hmac-secret            # подпись /internal/print-callback
NST_DOCUMENT_TOKEN_TTL_SECONDS=1800                 # TTL токена document_url, см. оговорку в разделе 5.2

# ── Auth — раздел 8 ─────────────────────────────────────────
# MVP-этап (раздел 11, M3): один статический ключ вместо полноценного JWT/BC
SERVICE_API_KEY=changeme-long-random-key
# Подключается на срезе `print` (раздел 11), когда появляется реальная BC-интеграция:
# BC_OAUTH_CLIENT_ID=
...
# ── Эксплуатация — раздел 9 ─────────────────────────────────
DOCUMENT_RETENTION_DAYS=90                          # ops/retention/cleanup-documents.sh
LOCK_TTL_SECONDS=60                                 # базовый TTL Redis-лока перед продлением/heartbeat
RECONCILER_STALE_THRESHOLD_MINUTES=15               # порог для reconciler зависших print_jobs
